...
# en gardant le dernier resultat par (company, date_from, date_to) pendant
...
class CamlaitDashboard(models.TransientModel):
    ...
    def get_dashboard_data(self, date_from=None, date_to=None):
        cache_key = (self.env.company.id, date_from, date_to)
        cached = _DASHBOARD_CACHE.get(cache_key)
        if cached and (time.time() - cached[0]) < _DASHBOARD_CACHE_TTL:
            return cached[1]

        result = self._compute_dashboard_data(date_from, date_to)
    ...
    def _compute_dashboard_data(self, date_from=None, date_to=None):
        today = date.today()
    ...
    def _get_special_location_ids(self, keyword):
        ...
        Recherche dans tous les entrepots cibles, independamment du
        filtre magasin frais/sec/entrepot de _get_target_location_ids."""
        loc_ids = []
        for wh in self._get_target_warehouses():
            locs = self.env['stock.location'].search([
                ('id', 'child_of', wh.view_location_id.id),
    ...
    def _get_avaries_count(self):
        ...
        les emplacements 'Avarie' de chaque entrepot cible."""
        ...
        return len(set(q.product_id.id for q in quants))

    def _get_echantillons_count(self):
        """Nombre de produits actuellement presents (quantite > 0) dans
        les emplacements 'Echantillon' de chaque entrepot cible."""
    ...
    def action_open_avaries(self):
        ...
        de chaque entrepot cible (meme calcul que 'avarie_count' dans
        _get_stock() / _get_avaries_count())."""
        loc_ids = self._get_special_location_ids('avarie')
        return {
    ...
    def action_open_echantillons(self):
        ...
        'Echantillon' de chaque entrepot cible (meme calcul que
        'echantillon_count' dans _get_stock() / _get_echantillons_count())."""
        loc_ids = self._get_special_location_ids('echantillon')
        return {
